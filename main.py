...
async def upload_resume(
    ...
    try:
        ...
        profile = agent.run(student_id, name, email, tmp_path)
...
async def get_candidates_for_job(job_id: str, token: str = None):
    ...
    for m in matches:
        for match in m.get("matches", []):
            if match.get("job_id") == job_id or match.get("source_id") == job_id:
                match["student_id"] = m["student_id"]
                match["student_name"] = m["student_name"]
                job_matches.append(match)
    
    # Sort by score
    job_matches.sort(key=lambda x: x.get("match_score", 0), reverse=True)
    return job_matches
